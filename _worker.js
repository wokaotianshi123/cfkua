...
export default {
    async fetch(request, env, ctx) {
        ...
        if (targetUrl.hostname.endsWith('doubanio.com')) {
            ...
            const imageRes = await fetch(targetUrl.href, { headers: doubanHeaders });
            if (!imageRes.ok) return new Response('Failed to fetch from Douban', { status: imageRes.status });

            const newHeaders = new Headers(imageRes.headers);
            newHeaders.set('Access-Control-Allow-Origin', '*');
            newHeaders.set('Cache-Control', 'public, s-maxage=2592000, max-age=604800'); // 边缘 30 天，浏览器 7 天
            newHeaders.delete('Set-Cookie');

            const response = new Response(imageRes.body, { status: imageRes.status, headers: newHeaders });
        ...
        if (el.tagName === "img") {
            ...
            if (srcset) {
                el.setAttribute("srcset", srcset.split(",").map(p => {
                    ...
                    try { parts[0] = this.proxy + "/" + new URL(parts[0], this.target).href; return parts.join(" "); } catch(e) { return p; }
                }).join(", "));
            }
...
function getRootHtml() {
    return `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Secure Proxy</title><style>body{margin:0;height:100vh;display:flex;align-items:center;justify-content:center;background:#f0f2f5;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif}.card{background:#fff;padding:2rem;border-radius:12px;box-shadow:0 8px 24px rgba(0,0,0,0.1);width:100%;max-width:400px}.title{text-align:center;font-size:1.5rem;margin-bottom:1.5rem;color:#1a1a1a;font-weight:600}input{width:100%;padding:12px;margin-bottom:1rem;border:1px solid #ddd;border-radius:6px;box-sizing:border-box;outline:none;transition:border-color .2s}input:focus{border-color:#0070f3}button{width:100%;padding:12px;background:#0070f3;color:#fff;border:none;border-radius:6px;cursor:pointer;font-weight:600;transition:background .2s}button:hover{background:#0051a2}@media(prefers-color-scheme:dark){body{background:#121212}.card{background:#1e1e1e}input{background:#2c2c2c;border-color:#444;color:#fff}.title{color:#fff}}</style></head><body><div class="card"><div class="title">Proxy Everything</div><form onsubmit="event.preventDefault();const u=document.getElementById('u').value.trim();if(u)window.open(location.origin+'/'+encodeURIComponent(u),'_blank')"><input type="text" id="u" placeholder="输入目标网址 (e.g., https://google.com)" required><button type="submit">跳转</button></form></div></body></html>`;
}
